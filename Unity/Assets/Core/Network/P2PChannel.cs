...
namespace ET
{
	...
	public class P2PChannel : AChannel
	{
		public static readonly Dictionary<IntPtr, P2PChannel> KcpPtrChannels = new Dictionary<IntPtr, P2PChannel>();
		...
		// connect
		public P2PChannel(long id, uint localConn, Socket socket, IPEndPoint remoteEndPoint, P2PService P2PService)
		{
			...
			this.socket = socket;
			this.lastRecvTime = P2PService.TimeNow;
			this.CreateTime = P2PService.TimeNow;
		...
		// accept
		public P2PChannel(long id, uint localConn, uint remoteConn, Socket socket, IPEndPoint remoteEndPoint, P2PService P2PService)
		{
			this.Id = id;
		...
		private void Connect()
		{
			try
			{
				...
				Log.Info($"P2PChannel connect {this.Id} {this.LocalConn} {this.RemoteConn} {this.RealAddress} {this.socket.LocalEndPoint}");
		...
		public void Update()
		{
			...
			if (!this.IsConnected)
			{
				...
				{
					Log.Error($"P2PChannel connect timeout: {this.Id} {this.RemoteConn} {timeNow} {this.CreateTime} {this.ChannelType} {this.RemoteAddress}");
		...
		public void HandleRecv(byte[] date, int offset, int length)
		{
			...
			while (true)
			{
				...
				if (this.needReadSplitCount > 0) // 说明消息分片了
				{
					...
					if (n != count)
					{
						Log.Error($"P2PChannel read error1: {this.LocalConn} {this.RemoteConn}");
					...
					{
						Log.Error($"P2PChannel read error2: {this.LocalConn} {this.RemoteConn}");
				...
				else
				{
					...
					if (n == 8)
					{
						...
						if (headInt == 0)
						{
							...
							{
								Log.Error($"P2PChannel read error3: {this.needReadSplitCount} {this.LocalConn} {this.RemoteConn}");
